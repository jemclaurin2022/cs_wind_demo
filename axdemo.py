...
conn = sql.Connect ( host = 'localhost', password = '' ,user = 'root', port = 4306, db='demo')

app = dash.Dash('ax-streaming-wind-app')
server = app.server

app.layout = html.Div([
    html.Div([
        html.H2("MariaDB AX Streaming Demo"),
        html.Img(src=""),
    ], className='banner'),
    html.Div([
        html.Div([
...
def gen_wind_speed(interval):
    ...
    df = pd.read_sql_query('SELECT Speed, SpeedError, Direction from Wind where\
                            rowid > "{}" AND rowid <= "{}";'
                            .format(total_time-200, total_time), conn)
